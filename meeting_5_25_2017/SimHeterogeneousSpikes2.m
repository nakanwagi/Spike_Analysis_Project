function SpikeTimes = SimHeterogeneousSpikes2(tstart,...,
...
f=@(x, sig, maxR) maxR*exp(-((mod(x+pi, 2*pi)-pi)).^2./(2.*sig.^2));
...
maxRate = rand(1, ncols)*20; %the maximum firing rate helps to ensure that we have many spiketimes for each neuron
...
for i = 1:ncols
    ...
    % heterogeneous sigma
    FireRate(:,i) = f(theta - center(i), sigma(i), maxRate(i)); 
