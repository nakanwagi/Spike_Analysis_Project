...
load('/project/agwan003/Sim_FRgplvm_noisy.mat');
...
FRgplvmEV = cell(1, size(FRgplvm,2)); %FRpca is pca on simulated noisy FireRate
for i = 1:size(FRgplvm, 2) % i runs over the columns of mappedX2
    FRgplvmEV{i} = tsd(time, FRgplvm(:,i));
end

figure;
a1 = subplot(2,2,1);
plot3(theta, FRgplvm(:,1), FRgplvm(:,2));
xlabel('theta');ylabel('eigvec 1');zlabel('eigvec 2');
title(a1, 'SimRatPos and top 2 eigvecs from simFRgplvm');
...
xb = linspace(min(x), max(x), 2*size(FRgplvm,2)); %range for x
yb = linspace(min(y), max(y), 2*size(FRgplvm,2)); %range for y
...
title(a2, 'Sim Pos and time:SimFRgplvm'); %SimFRgplvm stands for gaussin Process Latent variable model on simulated noisy FireRate
%print( 'MI_on_SimNoisyFRgKpca2','-depsc');

a3 = subplot(2,2,3);
plot3(x, y, FRgplvmEV{1}.data, 'r');
...
disp('computing Mutual Information for SimFRgplvm'); tic;

H_simFRgplvm=cell(1,32);
MI_simFRgplvm = cell(1,32);
Hnew_simFRgplvm = cell(1,32);

for iV = 1:size(FRgplvm, 2);
...
EV_FRgplvm = linspace(min(FRgplvmEV{iV}.data), max(FRgplvmEV{iV}.data), length(time));
    %compute a 3D hsitogram to estimate the joint entropy
    H_simFRgplvm{iV} = histcn([x, y, FRgplvmEV{iV}.data],xb, yb, EV_FRgplvm);
    % compute the normalizing constant;

% Matricize the  3D tensor (Histogram)  to speed up MATLAB computations

Hnew_simFRgplvm{iV} = reshape(H_simFRgplvm{iV}, [], size(H_simFRgplvm{iV},3)); %convert to 64^2*length(x.data)
%remove zeros so that log2 is defined
Hnew_simFRgplvm{iV}(Hnew_simFRgplvm{iV}==0)=nan;  %set all zero values to nans.


Hnew_simFRgplvm{iV} = Hnew_simFRgplvm{iV}./nansum(nansum(nansum(Hnew_simFRgplvm{iV}))); %normalize the probabilities


lh1 = log2(nansum(Hnew_simFRgplvm{iV},1));
lh2 = log2(nansum(Hnew_simFRgplvm{iV},2));
MI_simFRgplvm{iV} = nansum(nansum(Hnew_simFRgplvm{iV} .* bsxfun(@minus,bsxfun(@minus,log2(Hnew_simFRgplvm{iV}),lh1),lh2)));

end

% Create a Mutual Information vector by concatenating all the mutual Information
MI_simFRgplvm = cat(1, MI_simFRgplvm{:});


% Now plot the Mutual information
a4 = subplot(2,2,4);
p = plot(MI_simFRgplvm);
p.Marker = 'square';
xlabel('Eigen vector index'); ylabel('Mutual Information');
title(a4, 'Mutual Info Vs Eigindx : SimFRgplvm');
grid on;

print( 'MI_on_SimNoisyFRgplvm4','-depsc');
