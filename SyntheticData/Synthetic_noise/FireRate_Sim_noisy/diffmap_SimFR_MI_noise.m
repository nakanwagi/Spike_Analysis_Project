...
load('/project/agwan003/Sim_noisyFRdiffmap.mat');
...
FRdiffmapEV = cell(1, size(FRdiffmap,2)); %FRdiffmap is diffision maps on simulated noisy FireRate
for i = 1:size(FRdiffmap, 2) % i runs over the columns of mappedX2
    FRdiffmapEV{i} = tsd(time, FRdiffmap(:,i));
end

figure;
a1 = subplot(2,2,1);
plot3(theta, FRdiffmap(:,1), FRdiffmap(:,2));
xlabel('theta');ylabel('eigvec 1');zlabel('eigvec 2');
title(a1, 'SimRatPos and top 2 eigvecs from simFRdiffmap');
%print( 'MI_on_SimNoisyFRdiffmap1','-depsc');
...
xb = linspace(min(x), max(x), 2*size(FRdiffmap,2)); %range for x
yb = linspace(min(y), max(y), 2*size(FRdiffmap,2)); %range for y
...
title(a2, 'Sim Pos and time:SimFRdiffmap'); %SimFRpca stands for pca on simulated noisy FireRate
%print( 'MI_on_SimNoisyFRdiffmap2','-depsc');


a3 = subplot(2,2,3);
plot3(x, y, FRdiffmapEV{1}.data, 'r');
xlabel('cos(modtheta)');ylabel('sin(modtheta)');zlabel('1st eigvec');
title(a3, 'Sim Pos and 1st eigvec: simFRdiffmap');
%print( 'MI_on_SimNoisyFRdiffmap3','-depsc');
...
disp('computing Mutual Information for SimFRdiffmap'); tic;

H_simFRdiffmap=cell(1,32);
MI_simFRdiffmap = cell(1,32);
Hnew_simFRdiffmap = cell(1,32);

for iV = 1:size(FRdiffmap, 2);
...
EV_FRdiffmap = linspace(min(FRdiffmapEV{iV}.data), max(FRdiffmapEV{iV}.data), length(time));
    %compute a 3D hsitogram to estimate the joint entropy
    H_simFRdiffmap{iV} = histcn([x, y, FRdiffmapEV{iV}.data],xb, yb, EV_FRdiffmap);
    % compute the normalizing constant;

% Matricize the  3D tensor (Histogram)  to speed up MATLAB computations

Hnew_simFRdiffmap{iV} = reshape(H_simFRdiffmap{iV}, [], size(H_simFRdiffmap{iV},3)); %convert to 64^2*length(x.data)
%remove zeros so that log2 is defined
Hnew_simFRdiffmap{iV}(Hnew_simFRdiffmap{iV}==0)=nan;  %set all zero values to nans.


Hnew_simFRdiffmap{iV} = Hnew_simFRdiffmap{iV}./nansum(nansum(nansum(Hnew_simFRdiffmap{iV}))); %normalize the probabilities


lh1 = log2(nansum(Hnew_simFRdiffmap{iV},1));
lh2 = log2(nansum(Hnew_simFRdiffmap{iV},2));
MI_simFRdiffmap{iV} = nansum(nansum(Hnew_simFRdiffmap{iV} .* bsxfun(@minus,bsxfun(@minus,log2(Hnew_simFRdiffmap{iV}),lh1),lh2)));

end

% Create a Mutual Information vector by concatenating all the mutual Information
MI_simFRdiffmap = cat(1, MI_simFRdiffmap{:});


% Now plot the Mutual information
a4 = subplot(2,2,4);
p = plot(MI_simFRdiffmap);
p.Marker = 'square';
xlabel('Eigen vector index'); ylabel('Mutual Information');
title(a4, 'Mutual Info Vs Eigindx : SimFRdiffmap');
grid on;

print( 'MI_on_SimNoisyFRdiffmap4','-depsc');
