...
load('/project/agwan003/Sim_FRlKpca_noisy.mat');
...
FRlKpcaEV = cell(1, size(FRlKpca,2)); %FRlKpca is linear kernel pca on simulated noisy FireRate
for i = 1:size(FRlKpca, 2) % i runs over the columns of mappedX2
    FRlKpcaEV{i} = tsd(time, FRlKpca(:,i));
end

figure;
a1 = subplot(2,2,1);
plot3(theta, FRlKpca(:,1), FRlKpca(:,2));
xlabel('theta');ylabel('eigvec 1');zlabel('eigvec 2');
title(a1, 'SimRatPos and top 2 eigvecs from simFRlKpca');
%print( 'MI_on_SimNoisyFRlKpca1','-depsc');
...
xb = linspace(min(x), max(x), 2*size(FRlKpca,2)); %range for x
yb = linspace(min(y), max(y), 2*size(FRlKpca,2)); %range for y
...
title(a2, 'Sim Pos and time:SimFRlKpca'); %SimFRpca stands for pca on simulated noisy FireRate
%print( 'MI_on_SimNoisyFRlKpca2','-depsc');


%%
a3 = subplot(2,2,3);
plot3(x, y, FRlKpcaEV{1}.data, 'r');
xlabel('cos(modtheta)');ylabel('sin(modtheta)');zlabel('1st eigvec');
title(a3, 'Sim Pos and 1st eigvec');
%print( 'MI_on_SimNoisyFRlKpca3','-depsc');
...
disp('computing Mutual Information for SimFRlKpca'); tic;

H_simFRlKpca=cell(1,32);
MI_simFRlKpca = cell(1,32);
Hnew_simFRlKpca = cell(1,32);

for iV = 1:size(FRlKpca, 2);
...
EV_FRlKpca = linspace(min(FRlKpcaEV{iV}.data), max(FRlKpcaEV{iV}.data), length(time));
    %compute a 3D hsitogram to estimate the joint entropy
    H_simFRlKpca{iV} = histcn([x, y, FRlKpcaEV{iV}.data],xb, yb, EV_FRlKpca);
    % compute the normalizing constant;

% Matricize the  3D tensor (Histogram)  to speed up MATLAB computations

Hnew_simFRlKpca{iV} = reshape(H_simFRlKpca{iV}, [], size(H_simFRlKpca{iV},3)); %convert to 64^2*length(x.data)
%remove zeros so that log2 is defined
Hnew_simFRlKpca{iV}(Hnew_simFRlKpca{iV}==0)=nan;  %set all zero values to nans.


Hnew_simFRlKpca{iV} = Hnew_simFRlKpca{iV}./nansum(nansum(nansum(Hnew_simFRlKpca{iV}))); %normalize the probabilities


lh1 = log2(nansum(Hnew_simFRlKpca{iV},1));
lh2 = log2(nansum(Hnew_simFRlKpca{iV},2));
MI_simFRlKpca{iV} = nansum(nansum(Hnew_simFRlKpca{iV} .* bsxfun(@minus,bsxfun(@minus,log2(Hnew_simFRlKpca{iV}),lh1),lh2)));

end

% Create a Mutual Information vector by concatenating all the mutual Information
MI_simFRlKpca = cat(1, MI_simFRlKpca{:});


% Now plot the Mutual information
a4 = subplot(2,2,4);
p = plot(MI_simFRlKpca);
p.Marker = 'square';
xlabel('Eigen vector index'); ylabel('Mutual Information');
title(a4, 'Mutual Info Vs Eigindx : SimFRlKpca');
grid on;

print( 'MI_on_SimNoisyFRlKpca4','-depsc');
