...
load('/project/agwan003/Sim_FRgKpca_noisy.mat');
...
FRgKpcaEV = cell(1, size(FRgKpca,2)); %FRgKpca is gaussian kernel pca on simulated noisy FireRate
for i = 1:size(FRgKpca, 2) % i runs over the columns of mappedX2
    FRgKpcaEV{i} = tsd(time, FRgKpca(:,i));
end

figure;
a1 = subplot(2,2,1);
plot3(theta, FRgKpca(:,1), FRgKpca(:,2));
xlabel('theta');ylabel('eigvec 1');zlabel('eigvec 2');
title(a1, 'SimRatPos and top 2 eigvecs from simFRgKpca');
%print( 'MI_on_SimNoisyFRgKpca1','-depsc');
...
xb = linspace(min(x), max(x), 2*size(FRgKpca,2)); %range for x
yb = linspace(min(y), max(y), 2*size(FRgKpca,2)); %range for y
...
title(a2, 'Sim Pos and time:SimFRgKpca'); %SimFRpca stands for pca on simulated noisy FireRate
%print( 'MI_on_SimNoisyFRgKpca2','-depsc');



%figure;
a3 = subplot(2,2,3);
plot3(x, y, FRgKpcaEV{1}.data, 'r');
xlabel('cos(modtheta)');ylabel('sin(modtheta)');zlabel('1st eigvec');
title(a3, 'Sim Pos and 1st eigvec');
%print('MI_on_SimNoisyFRgKpca3','-depsc');
...
disp('computing Mutual Information for SimgFRgKpca'); tic;

H_simFRgKpca=cell(1,32);
MI_simFRgKpca = cell(1,32);
Hnew_simFRgKpca = cell(1,32);

for iV = 1:size(FRgKpca, 2);
...
EV_FRgKpca = linspace(min(FRgKpcaEV{iV}.data), max(FRgKpcaEV{iV}.data), length(time));
    %compute a 3D hsitogram to estimate the joint entropy
    H_simFRgKpca{iV} = histcn([x, y, FRgKpcaEV{iV}.data],xb, yb, EV_FRgKpca);
    % compute the normalizing constant;

% Matricize the  3D tensor (Histogram)  to speed up MATLAB computations

Hnew_simFRgKpca{iV} = reshape(H_simFRgKpca{iV}, [], size(H_simFRgKpca{iV},3)); %convert to 64^2*length(x.data)
%remove zeros so that log2 is defined
Hnew_simFRgKpca{iV}(Hnew_simFRgKpca{iV}==0)=nan;  %set all zero values to nans.


Hnew_simFRgKpca{iV} = Hnew_simFRgKpca{iV}./nansum(nansum(nansum(Hnew_simFRgKpca{iV}))); %normalize the probabilities


lh1 = log2(nansum(Hnew_simFRgKpca{iV},1));
lh2 = log2(nansum(Hnew_simFRgKpca{iV},2));
MI_simFRgKpca{iV} = nansum(nansum(Hnew_simFRgKpca{iV} .* bsxfun(@minus,bsxfun(@minus,log2(Hnew_simFRgKpca{iV}),lh1),lh2)));

end

% Create a Mutual Information vector by concatenating all the mutual Information
MI_simFRgKpca = cat(1, MI_simFRgKpca{:});


% Now plot the Mutual informatio
a4 =  subplot(2,2,4);
p = plot(MI_simFRgKpca);
p.Marker = 'square';
xlabel('Eigen vector index'); ylabel('Mutual Information');
title(a4, 'Mutual Info Vs Eigindx : SimFRgKpca');
grid on;

print( 'MI_on_SimNoisyFRgKpca4','-depsc');
