...
load('/project/agwan003/Sim_PreVlKpca_noisy.mat');
...
preVlKpcaEV = cell(1, size(preVlKpca,2)); %preVlKpca is linear kernel pca on simulated noisy previous time
for i = 1:size(preVlKpca, 2) % i runs over the columns of preVlKpca
    preVlKpcaEV{i} = tsd(time, preVlKpca(:,i));
end

figure;
plot3(theta, preVlKpca(:,1), preVlKpca(:,2));
xlabel('theta');ylabel('eigvec 1');zlabel('eigvec 2');
title('SimRatPos and top 2 eigvecs from simpreVlKpca');
print( 'MI_on_SimNoisyPreVlKpca1','-depsc');
...
xb = linspace(min(x), max(x), 2*size(preVlKpca,2)); %range for x
yb = linspace(min(y), max(y), 2*size(preVlKpca,2)); %range for y
...
title('Sim Pos and time:SimPreVlKpca'); %SimPreVlKpca stands for linear kernel pca on simulated noisy prevtime
print( 'MI_on_SimNoisyPreVlKpca2','-depsc');




figure;
plot3(x, y, preVlKpcaEV{1}.data, 'r');
xlabel('cos(modtheta)');ylabel('sin(modtheta)');zlabel('1st eigvec');
title('Sim Pos and 1st eigvec');
print( 'MI_on_SimNoisyPreVlKpca3','-depsc');
...
disp('computing Mutual Information for SimPreVlKpca'); tic;

H_simPreVlKpca=cell(1,32);
MI_simPreVlKpca = cell(1,32);
Hnew_simPreVlKpca = cell(1,32);

for iV = 1:size(preVlKpca, 2);
    % I have only put 10 because it's easy to see the contributions of
    % each eigenvector on submatrix plot
    
EV_PreVlKpca = linspace(min(preVlKpcaEV{iV}.data), max(preVlKpcaEV{iV}.data), length(time));
    %compute a 3D hsitogram to estimate the joint entropy
    H_simPreVlKpca{iV} = histcn([x, y, preVlKpcaEV{iV}.data],xb, yb, EV_PreVlKpca);
    % compute the normalizing constant;

% Matricize the  3D tensor (Histogram)  to speed up MATLAB computations

Hnew_simPreVlKpca{iV} = reshape(H_simPreVlKpca{iV}, [], size(H_simPreVlKpca{iV},3)); %convert to 64^2*length(x.data)
%remove zeros so that log2 is defined
Hnew_simPreVlKpca{iV}(Hnew_simPreVlKpca{iV}==0)=nan;  %set all zero values to nans.


Hnew_simPreVlKpca{iV} = Hnew_simPreVlKpca{iV}./nansum(nansum(nansum(Hnew_simPreVlKpca{iV}))); %normalize the probabilities


lh1 = log2(nansum(Hnew_simPreVlKpca{iV},1));
lh2 = log2(nansum(Hnew_simPreVlKpca{iV},2));
MI_simPreVlKpca{iV} = nansum(nansum(Hnew_simPreVlKpca{iV} .* bsxfun(@minus,bsxfun(@minus,log2(Hnew_simPreVlKpca{iV}),lh1),lh2)));

end

% Create a Mutual Information vector by concatenating all the mutual Information
MI_simPreVlKpca = cat(1, MI_simPreVlKpca{:});


% Now plot the Mutual information
figure;
p = plot(MI_simPreVlKpca);
p.Marker = 'square';
xlabel('Eigen vector index'); ylabel('Mutual Information');
title('Mutual Info Vs Eigindx : SimPreVlKpca');
grid on;

print( 'MI_on_SimNoisyPreVlKpca4','-depsc');
