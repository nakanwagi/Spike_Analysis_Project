...
load('/project/agwan003/Sim_PreVgKpca_noisy.mat');
...
preVgKpcaEV = cell(1, size(preVgKpca,2)); %preVgKpca is gaussian kernel pca on simulated noisy previous time
for i = 1:size(preVgKpca, 2) % i runs over the columns of preVgKpca
    preVgKpcaEV{i} = tsd(time, preVgKpca(:,i));
end
...
xb = linspace(min(x), max(x), 2*size(preVgKpca,2)); %range for x
yb = linspace(min(y), max(y), 2*size(preVgKpca,2)); %range for y
...
disp('computing Mutual Information for SimgPreVgKpca'); tic;

H_simPreVgKpca=cell(1,32);
MI_simPreVgKpca = cell(1,32);
Hnew_simPreVgKpca = cell(1,32);

for iV = 1:size(preVgKpca, 2);
...
EV_preVgKpca = linspace(min(preVgKpcaEV{iV}.data), max(preVgKpcaEV{iV}.data), length(time));
    %compute a 3D hsitogram to estimate the joint entropy
    H_simPreVgKpca{iV} = histcn([x, y, preVgKpcaEV{iV}.data],xb, yb, EV_preVgKpca);
    % compute the normalizing constant;

% Matricize the  3D tensor (Histogram)  to speed up MATLAB computations

Hnew_simPreVgKpca{iV} = reshape(H_simPreVgKpca{iV}, [], size(H_simPreVgKpca{iV},3)); %convert to 64^2*length(x.data)
%remove zeros so that log2 is defined
Hnew_simPreVgKpca{iV}(Hnew_simPreVgKpca{iV}==0)=nan;  %set all zero values to nans.


Hnew_simPreVgKpca{iV} = Hnew_simPreVgKpca{iV}./nansum(nansum(nansum(Hnew_simPreVgKpca{iV}))); %normalize the probabilities


lh1 = log2(nansum(Hnew_simPreVgKpca{iV},1));
lh2 = log2(nansum(Hnew_simPreVgKpca{iV},2));
MI_simPreVgKpca{iV} = nansum(nansum(Hnew_simPreVgKpca{iV} .* bsxfun(@minus,bsxfun(@minus,log2(Hnew_simPreVgKpca{iV}),lh1),lh2)));

end

% Create a Mutual Information vector by concatenating all the mutual Information
MI_simPreVgKpca = cat(1, MI_simPreVgKpca{:});
