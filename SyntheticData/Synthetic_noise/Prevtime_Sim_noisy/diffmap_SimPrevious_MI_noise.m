...
load('/project/agwan003/Sim_PreVdiffmap_noisy.mat');
...
preVdiffmapEV = cell(1, size(preVdiffmap,2)); %PreVdiffmap is diffision maps on simulated noisy previous data
for i = 1:size(preVdiffmap, 2) % i runs over the columns of preVdiffmap
    preVdiffmapEV{i} = tsd(time, preVdiffmap(:,i));
end
...
xb = linspace(min(x), max(x), 2*size(preVdiffmap,2)); %range for x
yb = linspace(min(y), max(y), 2*size(preVdiffmap,2)); %range for y
...
disp('computing Mutual Information for SimPreVdiffmap'); tic;

H_simPreVdiffmap=cell(1,32);
MI_simPreVdiffmap = cell(1,32);
Hnew_simPreVdiffmap = cell(1,32);

for iV = 1:size(preVdiffmap, 2);
...
EV_preVdiffmap = linspace(min(preVdiffmapEV{iV}.data), max(preVdiffmapEV{iV}.data), length(time));
    %compute a 3D hsitogram to estimate the joint entropy
    H_simPreVdiffmap{iV} = histcn([x, y, preVdiffmapEV{iV}.data],xb, yb, EV_preVdiffmap);
    % compute the normalizing constant;

% Matricize the  3D tensor (Histogram)  to speed up MATLAB computations

Hnew_simPreVdiffmap{iV} = reshape(H_simPreVdiffmap{iV}, [], size(H_simPreVdiffmap{iV},3)); %convert to 64^2*length(x.data)
%remove zeros so that log2 is defined
Hnew_simPreVdiffmap{iV}(Hnew_simPreVdiffmap{iV}==0)=nan;  %set all zero values to nans.


Hnew_simPreVdiffmap{iV} = Hnew_simPreVdiffmap{iV}./nansum(nansum(nansum(Hnew_simPreVdiffmap{iV}))); %normalize the probabilities


lh1 = log2(nansum(Hnew_simPreVdiffmap{iV},1));
lh2 = log2(nansum(Hnew_simPreVdiffmap{iV},2));
MI_simPreVdiffmap{iV} = nansum(nansum(Hnew_simPreVdiffmap{iV} .* bsxfun(@minus,bsxfun(@minus,log2(Hnew_simPreVdiffmap{iV}),lh1),lh2)));

end

% Create a Mutual Information vector by concatenating all the mutual Information
MI_simPreVdiffmap = cat(1, MI_simPreVdiffmap{:});
