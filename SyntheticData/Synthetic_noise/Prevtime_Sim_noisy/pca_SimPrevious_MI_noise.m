...
load('/project/agwan003/Sim_PrevPca_noisy.mat');
...
prevPcaEV = cell(1, size(prevPca,2)); %prevPca is pca on simulated noisy previoustime
for i = 1:size(prevPca, 2) % i runs over the columns of prevPca
    prevPcaEV{i} = tsd(time, prevPca(:,i));
end
figure;
a1 = subplot(2,2,1);
plot3(theta, prevPca(:,1), prevPca(:,2));
xlabel('theta');ylabel('eigvec 1');zlabel('eigvec 2');
title(a1,'SimRatPos and top 2 eigvecs from simprevPca');
%print( 'MI_on_SimNoisyPreVpca1','-depsc');
...
xb = linspace(min(x), max(x), 2*size(prevPca,2)); %range for x
yb = linspace(min(y), max(y), 2*size(prevPca,2)); %range for y
...
title(a2, 'Sim Pos and time:SimpreVca'); %SimprevPca stands for pca on simulated noisy FireRate
%print('MI_on_SimNoisyPreVpca2','-depsc');



a3 = subplot(2,2,3);
plot3(x, y, prevPcaEV{1}.data, 'r');
xlabel('cos(modtheta)');ylabel('sin(modtheta)');zlabel('1st eigvec');
title(a3, 'Sim Pos and 1st eigvec');
%print( 'MI_on_SimNoisyPreVpca3','-depsc');
...
disp('computing Mutual Information for SimpreVpca'); tic;

H_simPreVpca=cell(1,32);
MI_simPreVpca = cell(1,32);
Hnew_simPreVpca = cell(1,32);

for iV = 1:size(prevPca, 2);
...
EV_PreVpca = linspace(min(prevPcaEV{iV}.data), max(prevPcaEV{iV}.data), length(time));
    %compute a 3D hsitogram to estimate the joint entropy
    H_simPreVpca{iV} = histcn([x, y, prevPcaEV{iV}.data],xb, yb, EV_PreVpca);
    % compute the normalizing constant;

% Matricize the  3D tensor (Histogram)  to speed up MATLAB computations

Hnew_simPreVpca{iV} = reshape(H_simPreVpca{iV}, [], size(H_simPreVpca{iV},3)); %convert to 64^2*length(x.data)
%remove zeros so that log2 is defined
Hnew_simPreVpca{iV}(Hnew_simPreVpca{iV}==0)=nan;  %set all zero values to nans.


Hnew_simPreVpca{iV} = Hnew_simPreVpca{iV}./nansum(nansum(nansum(Hnew_simPreVpca{iV}))); %normalize the probabilities


lh1 = log2(nansum(Hnew_simPreVpca{iV},1));
lh2 = log2(nansum(Hnew_simPreVpca{iV},2));
MI_simPreVpca{iV} = nansum(nansum(Hnew_simPreVpca{iV} .* bsxfun(@minus,bsxfun(@minus,log2(Hnew_simPreVpca{iV}),lh1),lh2)));

end

% Create a Mutual Information vector by concatenating all the mutual Information
MI_simPreVpca = cat(1, MI_simPreVpca{:});


% Now plot the Mutual information
a4 = subplot(2,2,4);
p = plot(MI_simPreVpca);
p.Marker = 'square';
xlabel('Eigen vector index'); ylabel('Mutual Information');
title(a4, 'Mutual Info Vs Eigindx : SimPreVpca');
grid on;

print( 'MI_on_SimNoisyPreVpca4','-depsc');
