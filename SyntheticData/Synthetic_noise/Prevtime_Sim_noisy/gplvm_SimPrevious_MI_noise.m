...
%load('/project/agwan003/matfiles_synthetic_noise_7_28_2017.mat')--loads
%all analyses on synthetic noisy data

%% Load data from the 1TB projects space

load('/project/agwan003/Sim_PreVgplvm_noisy.mat');
...
preVgplvmEV = cell(1, size(preVgplvm,2)); %preVpca is pca on simulated noisy previous time
for i = 1:size(preVgplvm, 2) % i runs over the columns of mappedX2
    preVgplvmEV{i} = tsd(time, preVgplvm(:,i));
end
...
xb = linspace(min(x), max(x), 2*size(preVgplvm,2)); %range for x
yb = linspace(min(y), max(y), 2*size(preVgplvm,2)); %range for y
...
disp('computing Mutual Information for SimPreVgplvm'); tic;

H_simPreVgplvm=cell(1,32);
MI_simPreVgplvm = cell(1,32);
Hnew_simPreVgplvm = cell(1,32);

for iV = 1:size(preVgplvm, 2);
...
EV_PreVgplvm = linspace(min(preVgplvmEV{iV}.data), max(preVgplvmEV{iV}.data), length(time));
    %compute a 3D hsitogram to estimate the joint entropy
    H_simPreVgplvm{iV} = histcn([x, y, preVgplvmEV{iV}.data],xb, yb, EV_PreVgplvm);
    % compute the normalizing constant;

% Matricize the  3D tensor (Histogram)  to speed up MATLAB computations

Hnew_simPreVgplvm{iV} = reshape(H_simPreVgplvm{iV}, [], size(H_simPreVgplvm{iV},3)); %convert to 64^2*length(x.data)
%remove zeros so that log2 is defined
Hnew_simPreVgplvm{iV}(Hnew_simPreVgplvm{iV}==0)=nan;  %set all zero values to nans.


Hnew_simPreVgplvm{iV} = Hnew_simPreVgplvm{iV}./nansum(nansum(nansum(Hnew_simPreVgplvm{iV}))); %normalize the probabilities


lh1 = log2(nansum(Hnew_simPreVgplvm{iV},1));
lh2 = log2(nansum(Hnew_simPreVgplvm{iV},2));
MI_simPreVgplvm{iV} = nansum(nansum(Hnew_simPreVgplvm{iV} .* bsxfun(@minus,bsxfun(@minus,log2(Hnew_simPreVgplvm{iV}),lh1),lh2)));

end

% Create a Mutual Information vector by concatenating all the mutual Information
MI_simPreVgplvm = cat(1, MI_simPreVgplvm{:});
