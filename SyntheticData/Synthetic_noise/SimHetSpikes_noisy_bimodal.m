function [SpikeTimes, FireRate] = SimHetSpikes_noisy_bimodal(tstart,...,
...
%---------sigma for second receptive field-----------------------------   
...
%----------- sigma  for first receptive field--------------------------

   sig_max1 = 1; 
   sig_min1 = 0.01;
   sigma1 = sig_min1 + rand(1, ncols)*(sig_max1 - sig_min1);
...
center1 = rand(1, ncols)*2*pi; % centers must be the same size as N
eta1 = 0.05; % parameter for std of noise added to each center
center1 = center1 + (eta1*2*pi)*randn(size(center1)); 
%add Gaussian noise with std = eta*2*pi to the centers
...
% Parameters for the max and min FireRate for first place field
...
% Parameters for max and min FireRate for second place field.

M1 = 20; % maximum firing rate added as a parameter.

S1 = 400; % parameter for the minimum firing rate

maxRate1 = rand(1, ncols)*M1; %the maximum firing rate helps to ensure that we have many spiketimes for each neuron
...
g=@(y,sig1,maxR1,minR1) ((maxR1-minR1).*exp(-((mod(y+pi, 2*pi)-pi)).^2./(2.*sig1.^2))) + minR1;


%%  Add  noise component-wise in the firing rate
%----------for first receptive field-------------------------------------
eta1 = 0.05; %parameter for std of noise for the firing rate
noise1 = (eta1*M)*randn(size(FireRate)); % std of gaussian noise = (eta2*M)
...
for i = 1:ncols
    ...
    % heterogeneous sigma
    FireRate(:,i) = f(theta - center(i), sigma(i), maxRate(i), minRate(i)) + g(theta - center1(i), sigma1(i), maxRate1(i), minRate1(i)) + noise1(i);
