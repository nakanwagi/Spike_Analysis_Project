...
load('/project/agwan003/SyntheticMatFiles_5_29_2017/matfiles_5_29_2017/mutual_info_clean_tau_0.9.mat');
...
xb = linspace(min(x), max(x), 2*size(mappedX2,2)); %range for x
yb = linspace(min(y), max(y), 2*size(mappedX2,2)); %range for y
...
% print the plot
print('Rat Position and EigVector 1','-dpng');
...
disp('computing Mutual Information'); tic;
...
for iV = 1:32
    ...
    eigVecb = linspace(min(eigVec{iV}.data), max(eigVec{iV}.data), length(time));
    %compute a 3D hsitogram to estimate the joint entropy
    H{iV} = histcn([x, y, eigVec{iV}.data],xb, yb, eigVecb);
    % compute the normalizing constant;

% Matricize the  3D tensor (Histogram)  to speed up MATLAB computations

Hnew{iV} = reshape(H{iV}, [], size(H{iV},3)); %convert to 64^2*length(x.data)
%remove zeros so that log2 is defined
Hnew{iV}(Hnew{iV}==0)=nan;  %set all zero values to nans.


Hnew{iV} = Hnew{iV}./nansum(nansum(nansum(Hnew{iV}))); %normalize the probabilities


lh1 = log2(nansum(Hnew{iV},1));
lh2 = log2(nansum(Hnew{iV},2));
MutualInf{iV} = nansum(nansum(Hnew{iV} .* bsxfun(@minus,bsxfun(@minus,log2(Hnew{iV}),lh1),lh2)));

end

% Create a Mutual Information vector by concatenating all the mutual Information
MI_clean = cat(1, MutualInf{:});
...
title('Mutual Information Vs Eigindex for clean synthetic data');
grid on;

print('MI clean synthetic data','-dpng');
